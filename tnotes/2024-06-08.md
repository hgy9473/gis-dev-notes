...
1、优先做订房通知功能。
- [ ] 08:22 
	Q: SQL join 字段重名导致查询出的字段值和数据库不一致。
	
	```sql
	select ord.order_id,  
	       ord.user_id,       ord.house_id,       ord.client_name,       ord.client_id_card,       ord.client_phone_number,       ord.order_price,       ord.checkin_time,       ord.checkout_time,       ord.stay_nights,       ord.status,       ord.out_trade_no,       ord.trade_state,       ord.prepay_info,       ord.create_by,       ord.create_time,       ord.update_by,       ord.update_time,       ord.remark,       house.house_name,       house.house_type,       house.photos,       coupon.id,       coupon.type,       coupon.order_id,       coupon.deducted_amount,       coupon.offset_amount,       coupon.status,       coupon.create_timefrom lvj_order ord  
	         left join lvj_house house on ord.house_id = house.house_id         left join lvj_coupon_records coupon on coupon.order_id = ord.order_id
	```
	S: 为字段设置别名。参考：https://blog.csdn.net/baidu_30809315/article/details/78581122
	
	#sql
- [ ] 08:24 
	Q: join 查询，没有符合 join 条件的数据导致查询结果为空。
